...
function assetsTab(c) {
  ...
  var cards = mine.map(function (a) {
    return "<div class='item'><div class='ihead'><h4>" + esc(a.title || "Untitled") + "</h4><span class='kind'>" + esc(a.kind || "Lore") + "</span></div>" +
      (a.image ? "<a class='asset-img-link' href='/api/files/" + esc(a.image.id) + "' target='_blank' rel='noopener'><img class='asset-img' src='/api/files/" + esc(a.image.id) + "' alt='" + esc(a.title || "") + "' loading='lazy'></a>" : "") +
      ...
      (isKeeper() ? "<div class='edit-row'><button class='btn btn-ghost btn-sm' data-act='edit-asset' data-id='" + esc(a.id) + "'>Edit</button>" +
        "<button class='btn btn-ghost btn-sm' data-act='asset-image' data-id='" + esc(a.id) + "'>" + (a.image ? "Change image" : "Add image") + "</button>" +
        (a.image ? "<button class='btn btn-blood btn-sm' data-act='del-asset-image' data-id='" + esc(a.id) + "'>Remove image</button>" : "") +
...
function uploadAssetImage(assetId) {
  var inp = document.createElement("input");
  inp.type = "file";
  inp.accept = "image/png,image/jpeg,image/webp,image/gif,.png,.jpg,.jpeg,.webp,.gif";
  inp.onchange = function () {
    var f = inp.files && inp.files[0]; if (!f) return;
    if (f.size > 8 * 1024 * 1024) { toast("That image is over 8 MB.", true); return; }
    toast("Uploading image…");
    var rd = new FileReader();
    rd.onload = function () {
      var ct = f.type || (/\.png$/i.test(f.name) ? "image/png" : "image/jpeg");
      act("POST", "/api/assets/" + assetId + "/image", { filename: f.name, contentType: ct, dataBase64: String(rd.result) }, "Image added.").catch(function () {});
    };
    rd.onerror = function () { toast("Couldn't read that image.", true); };
    rd.readAsDataURL(f);
  };
...
function handleAct(act_, id) {
  ...
  switch (act_) {
    ...
    case "asset-image": uploadAssetImage(id); break;
    case "del-asset-image": confirmDel("Remove this image from the asset?", function () { return act("DELETE", "/api/assets/" + id + "/image", null, "Image removed."); }); break;
